<template>
    <div class="relative intro-y">
       <div class="flex justify-between mb-3 items-center md:flex-row flex-col">
            <span class="text-2xl mb-3 md:mb-0">فهرست نمایندگی ها</span>
            <PrimaryButton v-if="hasPermission('representation_create')" @click="clearForm">
        ...
        <div class="overflow-y-auto border border-themeOverlyPrimary md:rounded-lg">
            <div v-if="representations.isLoading" class="flex justify-center mx-auto p-5">
                <LoadingIcon
                    icon="three-dots"
                    color="#000"
            ...
            <table v-else class="min-w-full divide-y divide-themeOverlyPrimary table-auto">
                ...
                <tr v-for="(item, index) in representations.data" class="even:bg-gray-50">
                    <td class="p-2 border">{{ item.name }}</td>
                    <td class="p-2 border">{{ item.phone }}</td>
                    <td class="p-2 border">{{ item.address }}</td>
                    <td class="p-2 border">
                        <div class="flex items-center justify-center">
                            <div v-if="hasPermission('representation_edit')" class="mx-1">
                                <a
        ...
        <Pagination
            ...
            :class="representations.data.length"
            :paginate="paginate"
...
export default defineComponent({
    ...
    methods: {
        ...
        searchFilters(filter) {
            this.representations.isLoading = true;
            this.axios
                .get(`/representations`, {
                    params: {
                        filter,
                ...
                .then(({data}) => {
                    this.representations.isLoading = false;
                    this.representations.data = data.data
