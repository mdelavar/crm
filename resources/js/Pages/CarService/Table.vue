<template>
    <div class="relative intro-y">
        <div class="flex justify-between mb-3 items-center md:flex-row flex-col">
            <span class="text-2xl mb-3 md:mb-0">فهرست اتوسرویس ها</span>
            <PrimaryButton v-if="hasPermission('car_service_create')" @click="clearForm">
                <vue-feather class="ml-2" size="20" type="plus"/>
                افزودن اتوسرویس جدید
            </PrimaryButton>
        </div>
        <Filter @search-items="searchFilters"></Filter>
        <div class="overflow-y-auto border border-themeOverlyPrimary md:rounded-lg">
            <div v-if="carServices.isLoading" class="flex justify-center mx-auto p-5">
                <LoadingIcon
                    icon="three-dots"
                    color="#000"
            ...
            <table v-else class="min-w-full divide-y divide-themeOverlyPrimary table-auto">
                ...
                <tr v-for="(item, index) in carServices.data" class="even:bg-gray-50">
...
export default defineComponent({
    ...
    methods: {
        searchFilters(filter) {
            this.carServices.isLoading = true;
            this.axios
                .get(`/carServices`, {
                    params: {
                        filter,
                ...
                .then(({data}) => {
                    this.carServices.isLoading = false;
                    this.carServices.data = data.data
