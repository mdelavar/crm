<template>
    <div class="relative intro-y">
        ...
        <div class="overflow-hidden border border-themeOverlyPrimary md:rounded-lg">
            ...
            <table v-else class="min-w-full divide-y divide-themeOverlyPrimary table-auto">
                ...
                <tr v-for="(item, index) in products.data" class="even:bg-gray-50">
                    <td class="border px-2"><img class="w-10 h-10 rounded-full object-cover my-2 mx-auto"
                                                 :src="item.pic ?? DefLogo" @error="setDefault"/></td>
                    <td class="p-2 border">{{ item.name }}</td>
                    <td class="p-2 border">{{ item.en_name }}</td>
                    <td class="p-2 border">{{ item.category.title }}</td>
                    <td class="p-2 border">{{ item.description }}</td>
                    <td class="p-2 border">
                        <div class="flex items-center justify-center">
                            <div v-if="hasPermission('products_edit')" class="mx-1">
                                <a
    ...
    <Modal @close="showModal = false" :show="showModal" max-width="2xl" :closeable="false">
        <EntryForm @close="showModal = false" @reload="reload" :edit_items="edit_items" :categories="categories"/>
    </Modal>
